...
def _apply_env_overrides(cfg: dict) -> dict:
    """允许用环境变量覆盖敏感配置，避免把密钥写进 config.toml。"""
    deepseek = cfg.setdefault("deepseek", {})
    bilibili = cfg.setdefault("bilibili", {})

    if os.environ.get("DEEPSEEK_API_KEY"):
        deepseek["api_key"] = os.environ["DEEPSEEK_API_KEY"]
    if os.environ.get("DEEPSEEK_BASE_URL"):
        deepseek["base_url"] = os.environ["DEEPSEEK_BASE_URL"]
    if os.environ.get("DEEPSEEK_MODEL"):
        deepseek["model"] = os.environ["DEEPSEEK_MODEL"]
    if os.environ.get("BILIBILI_COOKIE"):
        bilibili["cookie"] = os.environ["BILIBILI_COOKIE"]
    if os.environ.get("BILIBILI_UID"):
        bilibili["uid"] = os.environ["BILIBILI_UID"]
    if os.environ.get("BILIBILI_REFRESH_TOKEN"):
        bilibili["refresh_token"] = os.environ["BILIBILI_REFRESH_TOKEN"]
    return cfg
...
def load_config() -> dict:
    ...
    try:
        ...
        return _apply_env_overrides(merge(copy.deepcopy(DEFAULT_CONFIG), cfg))
